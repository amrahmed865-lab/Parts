...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── بعت لكل التوكنات دفعة واحدة (max 500 per FCM call) ──
    const chunks = [];
    ...
    if (badTokens.length > 0) {
      ...
      const toDelSnap = await db.collection('tokens')
        .where('token', 'in', badTokens.slice(0, 30)) // Firestore 'in' limit
        .get();
